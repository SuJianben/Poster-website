...
Assert-Contains $section "asset_url }}&psv=plain-frame-finish-v1-20260818" 'The CSS custom template must version its framing stylesheet to invalidate stale storefront caches.'
...
Assert-Contains $styles '.ps-css-frame[data-frame-style="white-wood"] .ps-css-frame__rail,' 'White Wood must use the shared plain frame finish.'
Assert-Contains $styles '.ps-css-frame[data-frame-style="black-wood"] .ps-css-frame__rail {' 'Black Wood must use the shared plain frame finish.'
Assert-Contains $styles 'background-image: none;' 'The plain White and Black frame finish must not render grain or surface gradients.'
